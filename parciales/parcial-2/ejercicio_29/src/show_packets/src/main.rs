...
use colored::*; //crate para colorear la salida estándar


//crate pnet, para procesar la información de las tramas
...
fn display_eth_frame(ethernet_packet: &EthernetPacket) { //función para imprimir una trama ethernet
    println!(
        "{}",
        format!(
            "Ethernet frame: {} -> {}",
            ethernet_packet.get_source(), //método para conseguir la dirección que envía la trama ethernet
            ethernet_packet.get_destination() //método para conseguir la dirección que recibe la trama ethernet
        )
    );
}

fn display_arp_packet(arp_packet: &ArpPacket) { //función para imprimir una trama arp
    match arp_packet.get_operation() { //tipo de operación: ARP Request; ARP Reply
...
fn display_ipv4_packet(ipv4_packet: &Ipv4Packet) { //función para imprimir una trama IPv4
    println!(
        "    {}",
        format!(
            "IP Datagram: {} -> {}, ttl: {}",
...
fn display_icmp_packet(icmp_packet: &IcmpPacket) {  //función para imprimir una trama ICMP
    println!(
        "        {}",
        format!(
            "ICMP Packet: {:?}, {:?}",
...
fn display_tcp_packet(tcp_packet: &TcpPacket, ipv4_packet: &Ipv4Packet) { //función para imprimir una trama TCP
...
fn display_udp_packet(udp_packet: &UdpPacket, ipv4_packet: &Ipv4Packet) { //función para imprimir una trama UDP
    println!(
        "        {}",
        format!(
            "UDP Packet: {}:{} -> {}:{}",
    ...
    while let Ok(packet) = cap.next_packet() {
        ...
        if let Some(ethernet_packet) = EthernetPacket::new(&packet.data) {
            ...
            match ethernet_packet.get_ethertype() {
                EtherTypes::Arp => {
                    // Si la trama Ethernet contiene un paquete que es un ARP, construye un pnet::ArpPacket con
